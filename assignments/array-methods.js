...
// Problem 1
//Calculate how many t-shirts we need in each size so we can place the order.

let shirtCount = runners.reduce( (tally, shirts) => {
  tally[shirts.shirt_size] = (tally[shirts.shirt_size] || 0) + 1 ;
  return tally;
} , {});
...
//Split the list into waves so we can make a race day schedule. Odd ID numbers will be in the first wave, even ID numbers will be in the second wave.

let waveNumber = {
  wave1: runners.filter(num => num.id % 2 === 1),
  wave2: runners.filter(num => num.id % 2 === 0)
}
...
//A local business has decided to match each runners donation. We need a list of the new donation amount for each runner.
